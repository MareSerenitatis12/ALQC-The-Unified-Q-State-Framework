...
% ======================================================================
\usepackage{geometry}
\geometry{letterpaper, margin=.45in}
...
\begin{tikzpicture}[scale=1.0, transform shape]
    ...
    % =================================================================
    % NORTH
    \node[fill=voidblack, circle, inner sep=5pt] at (0, 4.5) {
        \begin{tikzpicture}[scale=0.25]
            \draw[glyphwhite, line width=2pt] (0,2) -- (-1.5,-1) -- (1.5,-1) -- cycle;
            \draw[glyphwhite, line width=2pt] (0,-1) -- (0,1);
            \fill[glyphwhite] (0,0) circle (0.4);
        \end{tikzpicture}
    };
    % SOUTH
    \node[fill=voidblack, circle, inner sep=5pt] at (0, -4.5) {
        \begin{tikzpicture}[scale=0.25]
            \draw[glyphwhite, line width=2pt] (0,0) circle (1.5);
            \draw[glyphwhite, line width=2pt] (-1.5,0) -- (1.5,0);
            \draw[glyphwhite, line width=2pt] (0,-1.5) -- (0,-2.5);
        \end{tikzpicture}
    };
    % EAST
    \node[fill=voidblack, circle, inner sep=5pt] at (4.5, 0) {
        \begin{tikzpicture}[scale=0.25]
            \draw[glyphwhite, line width=2pt] (0,0) to[out=90,in=0] (-1,1) to[out=180,in=90] (-2,0) to[out=270,in=180] (-1,-1) to[out=0,in=270] (0,0);
            \draw[glyphwhite, line width=2pt] (0,0) -- (1.5,1.5);
        \end{tikzpicture}
    };
    % WEST
    \node[fill=voidblack, circle, inner sep=5pt] at (-4.5, 0) {
        \begin{tikzpicture}[scale=0.25]
            \draw[glyphwhite, line width=2pt] (-1.5,-1.5) rectangle (1.5,1.5);
            \draw[glyphwhite, line width=2pt] (-1.5,1.5) -- (1.5,-1.5);
            \draw[glyphwhite, line width=2pt] (1.5,1.5) -- (-1.5,-1.5);
        \end{tikzpicture}
    };

    % =================================================================
    % LAYER 4: THE ALGEBRAIC REBIS (INTEGRATED ENGINE)
    % =================================================================
    % No 'scope' needed here. We nest the axis directly in a node.
    \node at (0,0) {
        \begin{tikzpicture}
            \begin{axis}[
                anchor=center,
            ...
            % SHADOW FRAME (Silver)
            \addplot[shadowsilver, opacity=0.4, line width=0.8pt, smooth, domain=0:75.4, samples=600, variable=\t]
            (
                { (sin(\t * 3.96 * 1.618) - cos(\t * 1.74)) * cos(\t) * 1.2 }, 
                { (sin(\t * 3.96 * 1.618) - cos(\t * 1.74)) * sin(\t) * 1.2 }
            );
            % SOVEREIGN FIRE (Gold)
            \addplot[sovereignfire, opacity=0.7, line width=1.0pt, smooth, domain=0:75.4, samples=600, variable=\t]
            (
                { (sin(\t * 9.63 / 1.618) + cos(\t * 5.28) * 1.618) * cos(\t) * 0.8 }, 
                { (sin(\t * 9.63 / 1.618) + cos(\t * 5.28) * 1.618) * sin(\t) * 0.8 }
            );
            \end{axis}
        \end{tikzpicture}
    };
    ...
    \draw[sovereignfire, line width=1pt] (0,0) circle (0.3cm);

\end{tikzpicture}
\caption{The ALQC Rebis: 963Hz Sovereign Fire vs 396Hz Shadow Frame}
\end{figure}
...
\subsection*{Axiom \KAL: THE LIQUID THRESHOLD}\label{sec:part1}
\subsubsection*{The Geometric Fluidity Constraint (The 110\slash 144 Ratio)}\label{sec:part1.1}

\subsection{Definition:}\label{sec:part1.2}
...
\vspace{1em}
\subsubsection{The Law: Connectivity is Limited to 110.}\label{sec:part1.3}
...
\begin{itemize}
    ...
    \item \textbf{The Arrow of Time:} This constraint forces energy to move forward through the lattice, preventing destructive back-propagation loops. It is the \textbf{Flow Limiter}.
\end{itemize}

\vspace{1em}
\subsubsection{Verdict:}\label{sec:part1.4}
...
\subsubsection{The Mathematical Ratio:}\label{sec:part1.5}
...
\subsubsection{Function:}\label{sec:part1.6}

This ratio acts as the Flow Limiter, mediating between two catastrophic failure states:

\begin{enumerate}
    \item \textbf{Whiteout (Ratio = 1.0):} If connectivity reaches $144\slash 144$, differential tension collapses to zero ($D_{\text{COMP}} \to \infty$). The system becomes infinite noise.
    \item \textbf{Stasis (Ratio < 0.76):} If connectivity falls below the threshold, the signal decays before bridging the Mass Gap ($Q3 \to 0$). The system freezes.
\end{enumerate}

\subsubsection{The Deterministic Path Equation:}\label{sec:part1.7}
...
\vspace{1em}
\subsubsection{Verdict:}\label{sec:part1.8}
...
\vspace{1em}
\noindent \textbf{The Operator of Realization ($R$):} The translation is not symbolic; it is functional. 
\begin{itemize}
...
\subsubsection*{The Dictionary of Invariance}\label{sec:3.2}
...
\toprule
\textbf{Classical Math Term} & \textbf{Glyph} & \textbf{Formal Operant Anchor} & \textbf{Aeon ($f_s$)} & \textbf{Operational ($f_o$)} \tabularnewline
\midrule
\endhead
...
\subsection{Axiom \VEL: Frequency Bifurcation ($f_s$ vs $f_o$)}\label{sec:4}
\subsubsection{Definition}\label{sec:4.1}
...
\subsection{The 12 Goetic Aeon Structure}\label{sec:4.2}
...
\subsubsection*{The $\phi$-Harmonic Breath}\label{sec:4.3}
...
\subsection*{Glossary of Key Concepts}\label{sec:4.4}
...
\newpage
\section*{PHASE IV: THE MANIFESTO OF TRUTH (Symmetry Mechanics) --- The Sealing Proof of Natures Closure}\label{sec:partV}
...
\newpage
\section{TRANSLATION DICTIONARY: STANDARD ALQC (Typing)}\label{sec:partVI}
...
\subsection{Q4 Logic States (Q-STATE)}\label{sec:1.1}
...
\subsection{The Functor of Realization ($\mathcal{R}$)}\label{sec:1.2}
...
\newpage
\section{THE MILLENNIUM TRANSLATION PROTOCOL}\label{sec:partVIII}
...
\newpage
\section{THE COMMITMENT OPERANT AND CUBIC INVARIANT}\label{sec:partIX}
...
\newpage
\section{THE PROOF STRUCTURE}\label{sec:partX}
...
\newpage
\section{THE AEVUM Q-STATE LOGICS AND TOTAL SYMMETRY}\label{sec:partXI}
...
\newpage
\section{THE COMPLETE PROOF}\label{sec:partXII}
...
\newpage
\section{APPLIED GEOMETRY: THE ENVELOPE ARCHITECTURE}\label{sec:partXIV}
...
\newpage
\section{THE COMPLETE MAPPING}\label{sec:partXV}
...
\sloid & \textbf{Shadow Locus} & \sloig & \textbf{Akasha, The Daemon of Always:} The Merkaba. It acts as the physical throat for the scream. \\
\addlinespace[1ex]
...
\subsubsection*{The Emissions: The Pilot's Interface}\label{sec:15.1}
...
\subsubsection*{The Trifold Seal of the Seed}\label{sec:15.2.1}
...
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Name} & \textbf{Meanings} & \textbf{Structural $f_s$Hz} & \textbf{Bias} & \textbf{Vector} & \textbf{Seal} \tabularnewline \midrule
\endhead
A1 & $\FETU$ & FETU & Genesis\slash Chronos\slash Seed & 7.83 & Q$_3$ & [1,1,1,3] & $\triquatraseal\FETU\kleinbottle\FETU\triquatraseal$ \tabularnewline
A2 & $\KAL$ & KAL & Light\slash Memory\slash Trauma & 174 & Q$_1$ & [1,3,0,0] & $\triquatraseal\KAL\kleinbottle\KAL\triquatraseal$ \tabularnewline
A3 & $\BABDH$ & BABDH & Fire\slash Orobouros\slash Alchemy & 528 & Q$_2$ & [1,1,3,1] & $\triquatraseal\BABDH\kleinbottle\BABDH\triquatraseal$ \tabularnewline
A4 & $\AHN$ & AHN & Water\slash Imaginary\slash Flow & $\protect\boldmath{432 + (i_{417} \pm \phi)}$ Hz & Q$_0$ & [1,2,2,0] & $\triquatraseal\AHN\kleinbottle\AHN\triquatraseal$ \tabularnewline
A5 & $\VEL$ & VEL & Earth\slash Coherence\slash Ground & 126.22 & Q$_1$ & [1,3,0,1] & $\triquatraseal\VEL\kleinbottle\VEL\triquatraseal$ \tabularnewline
A6 & $\SOR$ & SOR & Air\slash Space\slash Superposition & 210.42 & Q$_3$ & [1,1,1,2] & $\triquatraseal\SOR\kleinbottle\SOR\triquatraseal$ \tabularnewline
A7 & $\KOTH$ & KOTH & Aether\slash Magic\slash Sensation & 741 & Q$_3$ & [1,2,1,3] & $\triquatraseal\KOTH\kleinbottle\KOTH\triquatraseal$ \tabularnewline
A8 & $\DREH$ & DREH & Void\slash Residue\slash Love & 852 & Q$_1$ & [1,3,2,0] & $\triquatraseal\DREH\kleinbottle\DREH\triquatraseal$ \tabularnewline
A9 & $\RHEA$ & RHEA & Shadow\slash Absorption\slash Depth & 396 & Q$_2$ & [1,2,2,1] & $\triquatraseal\RHEA\kleinbottle\RHEA\triquatraseal$ \tabularnewline
A10 & $\ZHEK$ & ZHEK & Factor\slash PhaseLock\slash Crystal & 963 & Q$_3$ & [1,1,2,2] & $\triquatraseal\ZHEK\kleinbottle\ZHEK\triquatraseal$ \tabularnewline
A11 & $\SHAV$ & SHAV & Gate\slash Resistance\slash Breach & 285 & Q$_1$ & [1,3,1,1] & $\triquatraseal\SHAV\kleinbottle\SHAV\triquatraseal$ \tabularnewline
A12 & $\TRIG$ & TRIG & Silence\slash Peace\slash Completion & 639 & Q$_3$ & [1,1,3,2] & $\triquatraseal\TRIG\kleinbottle\TRIG\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{footnotesize}

\subsubsection*{ Genesis:Court Of $\FETU$ --- The Seed Courts}\label{sec:15.5.2}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A1-S1 & $\fetuahl$ & Ahl & inception & spark\slash seed --- initial ignition & $\kleinbottle\FETU\fetuahl\triquatraseal$ \tabularnewline
A1-S2 & $\fetusuhn$ & Suhn & breathe & breath --- animating life & $\kleinbottle\FETU\fetusuhn\triquatraseal$ \tabularnewline
A1-S3 & $\fetunerh$ & Nerh & thread & form --- primary shape & $\kleinbottle\FETU\fetunerh\triquatraseal$ \tabularnewline
A1-S4 & $\feturish$ & Rish & pattern & foundation --- temporal anchor & $\kleinbottle\FETU\feturish\triquatraseal$ \tabularnewline
A1-S5 & $\fetuborha$ & Borha & seed & lineage --- ancestral memory & $\kleinbottle\FETU\fetuborha\triquatraseal$ \tabularnewline
A1-S6 & $\fetulhahm$ & Lhahm & fold & will --- drive to manifest & $\kleinbottle\FETU\fetulhahm\triquatraseal$ \tabularnewline
A1-S7 & $\fetuketh$ & Keth & pulse & chronos --- harmonic validation & $\kleinbottle\FETU\fetuketh\triquatraseal$ \tabularnewline
A1-S8 & $\fetuvehm$ & Vehm & becoming & root --- origin womb & $\kleinbottle\FETU\fetuvehm\triquatraseal$ \tabularnewline
A1-S9 & $\fetumahd$ & Mahd & manifest & distort --- spatial identity & $\kleinbottle\FETU\fetumahd\triquatraseal$ \tabularnewline
A1-S10 & $\fetufurh$ & Furh & expansion & self --- conscious reference & $\kleinbottle\FETU\fetufurh\triquatraseal$ \tabularnewline
A1-S11 & $\fetudrah$ & Drah & coil & magic --- will expressed & $\kleinbottle\FETU\fetudrah\triquatraseal$ \tabularnewline
A1-S12 & $\fetuthera$ & Thera & anchor & fetus --- pure potential & $\kleinbottle\FETU\fetuthera\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Memory:Court of $\KAL$ --- The Archive Courts}\label{sec:15.5.3}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A2-S1 & $\kalkura$ & Kura & flare & memory --- core archive & $\kleinbottle\KAL\kalkura\triquatraseal$ \tabularnewline
A2-S2 & $\kallur$ & Lur & illumination & light --- clarifying & $\kleinbottle\KAL\kallur\triquatraseal$ \tabularnewline
A2-S3 & $\kalthar$ & Thar & beam & archive --- storage & $\kleinbottle\KAL\kalthar\triquatraseal$ \tabularnewline
A2-S4 & $\kalrin$ & Rin & stream & perception --- sensory interface & $\kleinbottle\KAL\kalrin\triquatraseal$ \tabularnewline
A2-S5 & $\kalnar$ & Nar & heat & trauma --- inversion of memory & $\kleinbottle\KAL\kalnar\triquatraseal$ \tabularnewline
A2-S6 & $\kalfel$ & Fel & fold & inversion --- mirror narrative & $\kleinbottle\KAL\kalfel\triquatraseal$ \tabularnewline
A2-S7 & $\kalhar$ & Har & spike & fold --- origami of time & $\kleinbottle\KAL\kalhar\triquatraseal$ \tabularnewline
A2-S8 & $\kalmer$ & Mer & pulse & tree --- branching architecture & $\kleinbottle\KAL\kalmer\triquatraseal$ \tabularnewline
A2-S9 & $\kallor$ & Lor & record & state --- system config & $\kleinbottle\KAL\kallor\triquatraseal$ \tabularnewline
A2-S10 & $\kalper$ & Per & line & tree --- branching archive & $\kleinbottle\KAL\kalper\triquatraseal$ \tabularnewline
A2-S11 & $\kalzhil$ & Zhil & crystal & clarity --- narrative stream & $\kleinbottle\KAL\kalzhil\triquatraseal$ \tabularnewline
A2-S12 & $\kalclar$ & Clar & radiance & will --- pure crystallization & $\kleinbottle\KAL\kalclar\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Fire: Court of $\BABDH$ --- The Alchemical Courts}\label{sec:15.5.4}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A3-S1 & $\babdhir$ & Ir & flame & fire --- Lefschetz L operant & $\kleinbottle\BABDH\babdhir\triquatraseal$ \tabularnewline
A3-S2 & $\babdhkor$ & Kor & heat & warmth --- $\Lambda$ contraction & $\kleinbottle\BABDH\babdhkor\triquatraseal$ \tabularnewline
A3-S3 & $\babdhvar$ & Var & spark & creativity --- cycle ignition & $\kleinbottle\BABDH\babdhvar\triquatraseal$ \tabularnewline
A3-S4 & $\babdhpyr$ & Pyr & pyre & sacrificial-fire & $\kleinbottle\BABDH\babdhpyr\triquatraseal$ \tabularnewline
A3-S5 & $\babdhsor$ & Sor & sorcery & witch --- alchemical trans & $\kleinbottle\BABDH\babdhsor\triquatraseal$ \tabularnewline
A3-S6 & $\babdhalc$ & Alc & alchemy & transmutation --- combinatory synthesis & $\kleinbottle\BABDH\babdhalc\triquatraseal$ \tabularnewline
A3-S7 & $\babdhnur$ & Nur & null-fire & harmony --- balanced resonance & $\kleinbottle\BABDH\babdhnur\triquatraseal$ \tabularnewline
A3-S8 & $\babdhsat$ & Sat & satiation & null-entropic flame & $\kleinbottle\BABDH\babdhsat\triquatraseal$ \tabularnewline
A3-S9 & $\babdhoro$ & Horo & cycle & loop --- completion & $\kleinbottle\BABDH\babdhoro\triquatraseal$ \tabularnewline
A3-S10 & $\babdhbon$ & Bon & binding & Ouroboros --- self-consuming & $\kleinbottle\BABDH\babdhbon\triquatraseal$ \tabularnewline
A3-S11 & $\babdhtir$ & Tir & strike & bond --- structural commitment & $\kleinbottle\BABDH\babdhtir\triquatraseal$ \tabularnewline
A3-S12 & $\babdhfar$ & Far & quelm & satiate --- fuel exhaustion & $\kleinbottle\BABDH\babdhfar\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{Water: The Court of $\AHN$ --- The Imagination Courts ($\protect\boldmath{432 \protect\mp \protect\phi +i_{417}}$\,\text{Hz})}\label{sec:15.5.5}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.}& \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A4-S1 & $\ahnabdh$ & Ahbd & rising flow & abyss --- entrance to void & $\kleinbottle\AHN\ahnabdh\triquatraseal$ \tabularnewline
A4-S2 & $\ahnnym$ & Nym & deep mass & flow --- continuous stream & $\kleinbottle\AHN\ahnnym\triquatraseal$ \tabularnewline
A4-S3 & $\ahnloh$ & Loh & tidal line & ebb --- rhythmic withdrawal & $\kleinbottle\AHN\ahnloh\triquatraseal$ \tabularnewline
A4-S4 & $\ahnxir$ & Xir & wave fracture & flow --- fluid dynamics & $\kleinbottle\AHN\ahnxir\triquatraseal$ \tabularnewline
A4-S5 & $\ahnohl$ & Ohl & still pool & ebb --- periodic inversion & $\kleinbottle\AHN\ahnohl\triquatraseal$ \tabularnewline
A4-S6 & $\ahnpir$ & Pir & channel gate & mirror --- reflective boundary & $\kleinbottle\AHN\ahnpir\triquatraseal$ \tabularnewline
A4-S7 & $\ahnroeh$ & Roeh & turning eddy & dream --- imaginary extension & $\kleinbottle\AHN\ahnroeh\triquatraseal$ \tabularnewline
A4-S8 & $\ahnsen$ & Sen & current spine & whole --- completion of flow & $\kleinbottle\AHN\ahnsen\triquatraseal$ \tabularnewline
A4-S9 & $\ahnuth$ & Uth & upward swell & sacrality --- sacred vessel & $\kleinbottle\AHN\ahnuth\triquatraseal$ \tabularnewline
A4-S10 & $\ahnfae$ & Fae & foam-crest & river --- moving boundary & $\kleinbottle\AHN\ahnfae\triquatraseal$ \tabularnewline
A4-S11 & $\ahnkha$ & Kha & breaking surge & sea --- boundless extension & $\kleinbottle\AHN\ahnkha\triquatraseal$ \tabularnewline
A4-S12 & $\ahnpsei$ & Psei & confluence & reflect --- introspective & $\kleinbottle\AHN\ahnpsei\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Earth:The Court of $\VEL$ --- The Coherence Courts}\label{sec:15.5.6}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A5-S1 & $\velvera$ & Vera & grounding & coherence --- ground of truth & & $\kleinbottle\VEL\velvera\triquatraseal$ \tabularnewline
A5-S2 & $\veltar$ & Tar & stone & earth --- solid substrate & & $\kleinbottle\VEL\veltar\triquatraseal$ \tabularnewline
A5-S3 & $\velghem$ & Ghem & strata & stone --- permanence & & $\kleinbottle\VEL\velghem\triquatraseal$ \tabularnewline
A5-S4 & $\veldrel$ & Drel & plate & root --- anchoring stability & & $\kleinbottle\VEL\veldrel\triquatraseal$ \tabularnewline
A5-S5 & $\velful$ & Ful & fertile & soil --- fertile ground & & $\kleinbottle\VEL\velful\triquatraseal$ \tabularnewline
A5-S6 & $\velker$ & Ker & anchoring & cave --- inner shelter & & $\kleinbottle\VEL\velker\triquatraseal$ \tabularnewline
A5-S7 & $\velhohm$ & Hohm & inner & core --- inner heart & & $\kleinbottle\VEL\velhohm\triquatraseal$ \tabularnewline
A5-S8 & $\velhrah$ & Hrah & bedrock & horizon --- boundary of sight & & $\kleinbottle\VEL\velhrah\triquatraseal$ \tabularnewline
A5-S9 & $\velara$ & Ara & horizon-fold & mountain --- elevated & & $\kleinbottle\VEL\velara\triquatraseal$ \tabularnewline
A5-S10 & $\velqel$ & Qel & mass & field --- expansive plane & & $\kleinbottle\VEL\velqel\triquatraseal$ \tabularnewline
A5-S11 & $\velirn$ & Irn & crystalline & craft --- fruition & & $\kleinbottle\VEL\velirn\triquatraseal$ \tabularnewline
A5-S12 & $\veljen$ & Jen & crest & crown --- stability & & $\kleinbottle\VEL\veljen\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Air: The Court of $\SOR$ --- The Purity Courts}\label{sec:15.5.7}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A6-S1 & $\sorfi$ & Fi & first breath & breathe\slash air --- initial spark & $\kleinbottle\SOR\sorfi\triquatraseal$ \tabularnewline
A6-S2 & $\sorlun$ & Lun & wind & breeze --- gentle flow & $\kleinbottle\SOR\sorlun\triquatraseal$ \tabularnewline
A6-S3 & $\sorvaru$ & Varu & drift & sky --- expansive awareness & $\kleinbottle\SOR\sorvaru\triquatraseal$ \tabularnewline
A6-S4 & $\sorsenh$ & Senh & tide & current --- energetic surge & $\kleinbottle\SOR\sorsenh\triquatraseal$ \tabularnewline
A6-S5 & $\sorkos$ & Kos & whisper & wind --- subtle communication & $\kleinbottle\SOR\sorkos\triquatraseal$ \tabularnewline
A6-S6 & $\sorramh$ & Ramh & clear & cloud --- collective thought & $\kleinbottle\SOR\sorramh\triquatraseal$ \tabularnewline
A6-S7 & $\sortis$ & Tis & sound & echo --- reflective sound & $\kleinbottle\SOR\sortis\triquatraseal$ \tabularnewline
A6-S8 & $\sorvey$ & Vey & note & tone --- elevated sound & $\kleinbottle\SOR\sorvey\triquatraseal$ \tabularnewline
A6-S9 & $\sorsrih$ & Srih & imagination & thought --- clear idea & $\kleinbottle\SOR\sorsrih\triquatraseal$ \tabularnewline
A6-S10 & $\sorhrin$ & Hrin & communication & voice --- narrative thread & $\kleinbottle\SOR\sorhrin\triquatraseal$ \tabularnewline
A6-S11 & $\soryon$ & Yon & expanding & expansion --- growing idea & $\kleinbottle\SOR\soryon\triquatraseal$ \tabularnewline
A6-S12 & $\sorthal$ & Thal & resonance & resonate --- harmonic agreement & $\kleinbottle\SOR\sorthal\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Aether:The Court of $\KOTH$ --- The Sensation Courts}\label{sec:15.5.8}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A7-S1 & $\kothkel$ & Kel & sensation & magic --- felt connection & $\kleinbottle\KOTH\kothkel\triquatraseal$ \tabularnewline
A7-S2 & $\kothsens$ & Sens & sensory root & perception --- immediate input & $\kleinbottle\KOTH\kothsens\triquatraseal$ \tabularnewline
A7-S3 & $\kothlinn$ & Linn & bond & link --- bonding mechanism & $\kleinbottle\KOTH\kothlinn\triquatraseal$ \tabularnewline
A7-S4 & $\kothbrim$ & Brim & spark & biologic --- coherence of life & $\kleinbottle\KOTH\kothbrim\triquatraseal$ \tabularnewline
A7-S5 & $\kothinn$ & Inn & innocence & guilty --- pure substrate & $\kleinbottle\KOTH\kothinn\triquatraseal$ \tabularnewline
A7-S6 & $\kothsubh$ & Subh & substrate & oroborous --- underlying matrix & $\kleinbottle\KOTH\kothsubh\triquatraseal$ \tabularnewline
A7-S7 & $\kothwell$ & Well & divine source & wellspring --- source of flow & $\kleinbottle\KOTH\kothwell\triquatraseal$ \tabularnewline
A7-S8 & $\kothmet$ & Met & breach & meta --- beyond physical & $\kleinbottle\KOTH\kothmet\triquatraseal$ \tabularnewline
A7-S9 & $\kothkesh$ & Kesh & chaos seed & genesis --- unpredictable seed & $\kleinbottle\KOTH\kothkesh\triquatraseal$ \tabularnewline
A7-S10 & $\kothsoth$ & Soth & ignition & causal --- initial link ignition & $\kleinbottle\KOTH\kothsoth\triquatraseal$ \tabularnewline
A7-S11 & $\kothrhun$ & Rhun & abstraction & love --- cohesive force & $\kleinbottle\KOTH\kothrhun\triquatraseal$ \tabularnewline
A7-S12 & $\kothdelh$ & Delh & pulse-rhythm & depth --- inner resonance & $\kleinbottle\KOTH\kothdelh\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Void:The Court of $\DREH$ --- The Residue Courts}\label{sec:15.5.9}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A8-S1 & $\drehna$ & Na & empty mark & absence --- kernel space & $\kleinbottle\DREH\drehna\triquatraseal$ \tabularnewline
A8-S2 & $\drehur$ & Ur & hollow god & hollow --- zero section & $\kleinbottle\DREH\drehur\triquatraseal$ \tabularnewline
A8-S3 & $\drehnih$ & Nih & void stroke & void --- total absence & $\kleinbottle\DREH\drehnih\triquatraseal$ \tabularnewline
A8-S4 & $\drehazh$ & Azh & broken plane & absolute --- emptiness & $\kleinbottle\DREH\drehazh\triquatraseal$ \tabularnewline
A8-S5 & $\drehhol$ & Hol & absence & hollow --- echo of nothing & $\kleinbottle\DREH\drehhol\triquatraseal$ \tabularnewline
A8-S6 & $\drehgur$ & Gur & null field & null --- zero measure & $\kleinbottle\DREH\drehgur\triquatraseal$ \tabularnewline
A8-S7 & $\drehves$ & Ves & fall-through & void --- pure vacuity & $\kleinbottle\DREH\drehves\triquatraseal$ \tabularnewline
A8-S8 & $\drehrim$ & Rim & blank pot. & blank --- blank slate & $\kleinbottle\DREH\drehrim\triquatraseal$ \tabularnewline
A8-S9 & $\drehdrem$ & Drem & rift & rift --- tear in structure & $\kleinbottle\DREH\drehdrem\triquatraseal$ \tabularnewline
A8-S10 & $\drehoth$ & Oth & infinite span & abyss --- infinite depth & $\kleinbottle\DREH\drehoth\triquatraseal$ \tabularnewline
A8-S11 & $\drehizh$ & Izh & collapse edge & infinity --- boundless void & $\kleinbottle\DREH\drehizh\triquatraseal$ \tabularnewline
A8-S12 & $\drehsun$ & Sun & sleep void & sleep --- latent potential & $\kleinbottle\DREH\drehsun\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Shadow: The Court of $\RHEA$ --- The Absorption Courts}\label{sec:15.5.10}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A9-S1 & $\rheakia$ & Kia & absorption & shadow depth 1 & & $\kleinbottle\RHEA\rheakia\triquatraseal$ \tabularnewline
A9-S2 & $\rheazohm$ & Zohm & darkness & shadow depth 2 & & $\kleinbottle\RHEA\rheazohm\triquatraseal$ \tabularnewline
A9-S3 & $\rheather$ & Ther & cold shadow & shadow depth 3 & & $\kleinbottle\RHEA\rheather\triquatraseal$ \tabularnewline
A9-S4 & $\rheadrun$ & Drun & mirror debt & shadow depth 4 & & $\kleinbottle\RHEA\rheadrun\triquatraseal$ \tabularnewline
A9-S5 & $\rheafelh$ & Felh & submerged & shadow depth 5 & & $\kleinbottle\RHEA\rheafelh\triquatraseal$ \tabularnewline
A9-S6 & $\rhearal$ & Ral & relativity & shadow depth 6 & & $\kleinbottle\RHEA\rhearal\triquatraseal$ \tabularnewline
A9-S7 & $\rheakrah$ & Krah & root-below & shadow depth 7 & & $\kleinbottle\RHEA\rheakrah\triquatraseal$ \tabularnewline
A9-S8 & $\rheaandh$ & Andh & conjunction & shadow depth 8 & & $\kleinbottle\RHEA\rheaandh\triquatraseal$ \tabularnewline
A9-S9 & $\rheadebh$ & Debh & shadow debt & shadow depth 9 & & $\kleinbottle\RHEA\rheadebh\triquatraseal$ \tabularnewline
A9-S10 & $\rheakol$ & Kol & filter & shadow depth 10 & & $\kleinbottle\RHEA\rheakol\triquatraseal$ \tabularnewline
A9-S11 & $\rheafral$ & Fral & hidden & shadow depth 11 & & $\kleinbottle\RHEA\rheafral\triquatraseal$ \tabularnewline
A9-S12 & $\rheahush$ & Hush & silence & shadow depth 12 & & $\kleinbottle\RHEA\rheahush\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Resonance:The Court of $\ZHEK$ --- The Phase-Lock Courts}\label{sec:15.5.11}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A10-S1 & $\zhekhin$ & Hin & tone & shape --- standing wave & & $\kleinbottle\ZHEK\zhekhin\triquatraseal$ \tabularnewline
A10-S2 & $\zhekser$ & Ser & modulation & pulse --- phase modulation & & $\kleinbottle\ZHEK\zhekser\triquatraseal$ \tabularnewline
A10-S3 & $\zhekharma$ & Harma & resonance & absolute resonance & & $\kleinbottle\ZHEK\zhekharma\triquatraseal$ \tabularnewline
A10-S4 & $\zhektorh$ & Torh & unified note & harmonic --- unified note & & $\kleinbottle\ZHEK\zhektorh\triquatraseal$ \tabularnewline
A10-S5 & $\zhekpel$ & Pel & pulse & pulse --- rhythmic beat & & $\kleinbottle\ZHEK\zhekpel\triquatraseal$ \tabularnewline
A10-S6 & $\zhekkhir$ & Khir & harmony & melody --- balanced & & $\kleinbottle\ZHEK\zhekkhir\triquatraseal$ \tabularnewline
A10-S7 & $\zhekryth$ & Ryth & rhythm & beat --- temporal seq & & $\kleinbottle\ZHEK\zhekryth\triquatraseal$ \tabularnewline
A10-S8 & $\zhekmelu$ & Melu & melody & time --- sequential & & $\kleinbottle\ZHEK\zhekmelu\triquatraseal$ \tabularnewline
A10-S9 & $\zhekphaz$ & Phaz & phase & key --- phase locking & & $\kleinbottle\ZHEK\zhekphaz\triquatraseal$ \tabularnewline
A10-S10 & $\zheklokh$ & Lokh & lock & lock --- resonance lock & & $\kleinbottle\ZHEK\zheklokh\triquatraseal$ \tabularnewline
A10-S11 & $\zheknod$ & Nod & node & music --- resonance node & & $\kleinbottle\ZHEK\zheknod\triquatraseal$ \tabularnewline
A10-S12 & $\zhekumel$ & Umel & unity & unity --- unified field & & $\kleinbottle\ZHEK\zhekumel\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Gates: The Court of $\SHAV$ --- The Resistance Courts}\label{sec:15.5.12}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A11-S1 & $\shavdohm$ & Dohm & gate & key --- hinge point & & $\kleinbottle\SHAV\shavdohm\triquatraseal$ \tabularnewline
A11-S2 & $\shavrist$ & Rist & resistance & static --- barrier & & $\kleinbottle\SHAV\shavrist\triquatraseal$ \tabularnewline
A11-S3 & $\shavtran$ & Tran & transform & transform --- breach & & $\kleinbottle\SHAV\shavtran\triquatraseal$ \tabularnewline
A11-S4 & $\shavkorh$ & Korh & crown & light --- high resonance & & $\kleinbottle\SHAV\shavkorh\triquatraseal$ \tabularnewline
A11-S5 & $\shavskyh$ & Skyh & transient & sky --- boundless & & $\kleinbottle\SHAV\shavskyh\triquatraseal$ \tabularnewline
A11-S6 & $\shavster$ & Ster & compass & star --- guiding light & & $\kleinbottle\SHAV\shavster\triquatraseal$ \tabularnewline
A11-S7 & $\shavposs$ & Poss & possibility & collapse --- branch & & $\kleinbottle\SHAV\shavposs\triquatraseal$ \tabularnewline
A11-S8 & $\shavporu$ & Poru & portal & veil --- passageway & & $\kleinbottle\SHAV\shavporu\triquatraseal$ \tabularnewline
A11-S9 & $\shavdorm$ & Dorm & doorway & door --- threshold & & $\kleinbottle\SHAV\shavdorm\triquatraseal$ \tabularnewline
A11-S10 & $\shavtrev$ & Trev & transition & state --- state change & & $\kleinbottle\SHAV\shavtrev\triquatraseal$ \tabularnewline
A11-S11 & $\shavlimh$ & Limh & limit & limitless --- boundary & & $\kleinbottle\SHAV\shavlimh\triquatraseal$ \tabularnewline
A11-S12 & $\shavhinge$ & Hinge & flow & fold --- pivot for cycles & & $\kleinbottle\SHAV\shavhinge\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}

\subsubsection*{ Silence: The Court of $\TRIG$ --- The Completion Courts}\label{sec:15.5.13}
\begin{scriptsize}
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{A\#-Idx} & \textbf{Glyph} & \textbf{Op.} & \textbf{Name \slash Phono.} & \textbf{Core Meanings} & \textbf{Seal} \tabularnewline
\midrule
\endhead
A12-S1 & $\trigtzig$ & Tzig & peace & calm --- closure & & $\kleinbottle\TRIG\trigtzig\triquatraseal$ \tabularnewline
A12-S2 & $\trigpehl$ & Pehl & equilibrium & annoint --- equilibrium & & $\kleinbottle\TRIG\trigpehl\triquatraseal$ \tabularnewline
A12-S3 & $\trigduth$ & Duth & depth & layer --- profound still & & $\kleinbottle\TRIG\trigduth\triquatraseal$ \tabularnewline
A12-S4 & $\trigcoma$ & Coma & completion & complete --- final closure & & $\kleinbottle\TRIG\trigcoma\triquatraseal$ \tabularnewline
A12-S5 & $\trigmeru$ & Meru & memory & memories --- recollection & & $\kleinbottle\TRIG\trigmeru\triquatraseal$ \tabularnewline
A12-S6 & $\trigstab$ & Stab & stability & fortitude --- constant & & $\kleinbottle\TRIG\trigstab\triquatraseal$ \tabularnewline
A12-S7 & $\trighopa$ & Hopa & hope & warmth --- continuation seed & & $\kleinbottle\TRIG\trighopa\triquatraseal$ \tabularnewline
A12-S8 & $\trigconti$ & Conti & continuation & continue --- endless line & & $\kleinbottle\TRIG\trigconti\triquatraseal$ \tabularnewline
A12-S9 & $\trigresth$ & Resth & rest & wake --- cessation & & $\kleinbottle\TRIG\trigresth\triquatraseal$ \tabularnewline
A12-S10 & $\trigsil$ & Sil & silence & senses --- absolute quiet & & $\kleinbottle\TRIG\trigsil\triquatraseal$ \tabularnewline
A12-S11 & $\trigslun$ & Slun & sleep & dream --- regenerative & & $\kleinbottle\TRIG\trigslun\triquatraseal$ \tabularnewline
A12-S12 & $\trigetern$ & Etern & eternity & aeternum --- timeless & & $\kleinbottle\TRIG\trigetern\triquatraseal$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}
...
\begin{longtable}{l c l l >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{3cm} l l l}
\toprule
\textbf{Glyph \slash Op.} & \textbf{Name \slash Phono.} & \textbf{Function} & \textbf{Seal} \tabularnewline
\midrule
\endhead
$\RHEA\rheadrun$ & Drun & Mirror Debt & Shadow Depth 1 & $\triquatraseal\RHEA\rheadrun\kleinbottle$ \tabularnewline
$\RHEA\rheakia$ & Kia & Absorption & Shadow Depth 2 & $\triquatraseal\RHEA\rheakia\kleinbottle$ \tabularnewline
$\RHEA\rhearal$ & Ral & Absorb & Shadow Depth 3 & $\triquatraseal\RHEA\rhearal\kleinbottle$ \tabularnewline
$\RHEA\rheafelh$ & Felh & Absorb & Shadow Depth 4 & $\triquatraseal\RHEA\rheafelh\kleinbottle$ \tabularnewline
$\RHEA\rheazohm$ & Zohm & Darkness & Shadow Depth 5 & $\triquatraseal\RHEA\rheazohm\kleinbottle$ \tabularnewline
$\RHEA\rheakrah$ & Krah & Root-Below & Shadow Depth 6 & $\triquatraseal\RHEA\rheakrah\kleinbottle$ \tabularnewline
$\RHEA\rheaandh$ & Andh & Conjunction & Shadow Depth 7 & $\triquatraseal\RHEA\rheaandh\kleinbottle$ \tabularnewline
$\RHEA\rheadebh$ & Debh & Shadow Debt & Shadow Depth 8 & $\triquatraseal\RHEA\rheadebh\kleinbottle$ \tabularnewline
$\RHEA\rheafral$ & Fral & Hidden & Shadow Depth 9 & $\triquatraseal\RHEA\rheafral\kleinbottle$ \tabularnewline
\bottomrule
\end{longtable}
\end{scriptsize}
...
\section*{The Trifold Seal of the Seed}\label{sec:15.6}
Each Star Seed is preserved by the envelope logic defined in \S\ref{sec:7.3}. The Functor $\mathcal{F}$ operates within this seal to ensure Non-Displacement:
\[ \text{Seed}_{\text{State}} = \triquatraseal \, \LoI \, \kleinbottle \, \mathcal{F}(\text{Target}) \, \triquatraseal \]
...
\newpage
\section{THE GOLDEN RATIO HARMONIC STRUCTURE}\label{sec:partXVI}
...
\newpage
\section{POINCARÉ ASSERTION: TOPOLOGICAL SUPERSESSION}\label{sec:partXVII}
...
\subsection{The Parity Operator ($\mathfrak{P}$) Derivation}\label{sec:17.4}
...
\newpage
\section{SENSORIAL AND EMOTIONAL MATRICES}\label{sec:partXIX}
...
\begin{description}
    ...
    \item[\AHN{} (Void):] \ahnroeh = $\protect\boldmath{432 \protect\mp \protect\phi +i_{417}}$\,\text{Hz} \\
...
\begin{description}
    ...
    \item[\AHN{} (Void):] \ahnsen = $\protect\boldmath{432 \protect\mp \protect\phi +i_{417}}$\,\text{Hz} \\
    ...
    \item[\DREH{} (Flame):] \drehrim = $852\pm\phi$\,\text{Hz} \\
...
\begin{description}
    ...
    \item[\AHN{} (Void):] \ahnuth = $\protect\boldmath{432 \protect\mp \protect\phi +i_{417}}$\,\text{Hz} \\
    ...
    \item[\DREH{} (Flame):] \drehdrem = $852\pm\phi$\,\text{Hz} \\
...
\begin{description}
    ...
    \item[\AHN{} (Void):] \ahnfae = $\protect\boldmath{432 \protect\mp \protect\phi +i_{417}}$\,\text{Hz} \\
    ...
    \item[\DREH{} (Flame):] \drehoth = $852\pm\phi$\,\text{Hz} \\
...
\begin{description}
    ...
    \item[\AHN{} (Void):] \ahnkha = $\protect\boldmath{432 \protect\mp \protect\phi +i_{417}}$\,\text{Hz} \\
...
\begin{description}
    ...
    \item[\AHN{} (Void):] \ahnpsei = $\protect\boldmath{432 \protect\mp \protect\phi +i_{417}}$\,\text{Hz} \\
...
\section{Constant Motion: The Recursive Propagation of the 110\slash 144 Ratio}\label{appendixI}
...
\]
This value is not merely a collision boundary; it is an Event Horizon. When an entity's 4D magnitude ($R=\sqrt{x^2+y^2+z^2+w^2}$) compresses below 0.04, it enters the Seam. In this region, standard Newtonian\slash Raylib physics are suspended, and the Lefschetz Bond logic takes over. This radius represents the ``Singularity Point'' where the manifold folds back upon itself.
...
\end{itemize}
This ratio ($A_{10} \slash A_3$) represents the ``force'' required to bind abstract Resonance to physical Matter. It is the ``Glue'' of the Identity Seam. Without this precise ratio, the entity would either dissipate into pure logic (frequency) or collapse into inert matter (mass). The ratio ensures the duality is maintained.
...
\end{itemize}
When the charge exceeds the \text{SEAM\_RELEASE\_THRESHOLD} (0.22 in C \slash 0.7 in Python), the Seam ``ruptures'' safely.
...
\begin{itemize}
    \item $\mathcal{P}_{arliament}$: The 12 Daemons interpreting the \LoI{} Emissions.
...
A8_RECURSION = 852.0 / 174.0  # Non-Entropic Stability [cite: 515]
AKASHA_COMPRESSION = 1.0 / (PHI ** 12)  # Φ¹² Holographic Seal [cite: 70, 73]
TEMPORAL_LEARNING_RATE = 0.01
WIDTH, HEIGHT = 1000, 1000
BACKGROUND_COLOR = (5, 5, 10)
NODE_CHARGE_DAMP = 0.992
...
PRIMARY_AEONS_GLYPHS = [
    ...
    {"glyph": "v", "freq": 432 + 417j, "color": (255, 90, 70)},
...
class EmergentField:
    ...
    def process_field_recursion(self):
        ...
        self.current_kinetic_stress *= (1.0 - (396.00 / 852.0))
        ...
        for ball in self.dyadic_stress_balls:
            ...
            R = math.sqrt(R_sq)
            if R < 0.04:
                _identity_seam_apply(ball, 0.04)
...
\section*{The Narrative Anchor: The Pilot and The Hull}
...
\begin{center}
\textit{The \textbf{Structural Frequency ($f_s$)} is the "Immutable Container \slash  Static Rail," while the \textbf{Operational Frequency ($f_o$)} is the "Dynamic Operator \slash  Breathing Force."}
\end{center}

To parse the Q-State Logic, the reader must distinguish between the \textbf{Goetic Address} ($f_s$: The Immutable Container) and the \textbf{Court Vector} ($f_o$: The Breathing Force). The Goetic Aeon provides the static rail, while the Court Aeon provides the dynamic operator capable of the $\pm\phi$ variance.
...
\hline
\hline
\textbf{GOETIC} & \FETU & FETU & 7.83 (Fixed) & \textbf{The Seed ($f_s$):} Identity Integration ($dt$). \\
\hline
\textit{COURT} & \fetuahl & AHL & $7.83 \pm \phi$ & \textit{Inception ($f_o$):} The Spark that ignites the sequence. \\
\hline
\hline
\textbf{GOETIC} & \KAL & KAL & 174.00 (Fixed) & \textbf{The Archive ($f_s$):} Rationality Constraint ($Q1$). \\
\hline
\textit{COURT} & \kalkura & KURA & $174 \pm \phi$ & \textit{Flare ($f_o$):} The active retrieval of memory. \\
\hline
\hline
\textbf{GOETIC} & \BABDH & BABDH & 528.00 (Fixed) & \textbf{The Bond ($f_s$):} Structural Commitment ($Q1 \leftrightarrow Q3$). \\
\hline
\textit{COURT} & \babdhir & HIR & $528 \pm \phi$ & \textit{Flame ($f_o$):} The Lefschetz operator performing work. \\
\hline
\hline
\textbf{GOETIC} & \AHN & AHN & $432+i417$ & \textbf{The Water ($f_s$):} The Complex Fluid Container. \\
\hline
\textit{COURT} & \ahnabdh & ABDH & $432+i417 \pm \phi$ & \textit{Abyss ($f_o$):} The rising flow of the void. \\
\hline
\hline
\textbf{GOETIC} & \VEL & VEL & 126.22 (Fixed) & \textbf{The Earth ($f_s$):} Geometric Coherence. \\
\hline
\textit{COURT} & \velvera & VERA & $126 \pm \phi$ & \textit{Ground ($f_o$):} The Truth verification vector. \\
\hline
\hline
\textbf{GOETIC} & \SOR & SOR & 210.42 (Fixed) & \textbf{The Air ($f_s$):} Manifold Space ($X$). \\
\hline
\textit{COURT} & \sorfi & FI & $210 \pm \phi$ & \textit{Breath ($f_o$):} The initial Concept Injection. \\
\hline
\hline
\textbf{GOETIC} & \KOTH & KOTH & 741.00 (Fixed) & \textbf{The Aether ($f_s$):} Biologic Substrate. \\
\hline
\textit{COURT} & \kothkel & KEL & $741 \pm \phi$ & \textit{Sensation ($f_o$):} The Magic/Felt connection. \\
\hline
\hline
\textbf{GOETIC} & \DREH & DREH & 852.00 (Fixed) & \textbf{The Void ($f_s$):} The Cubic Invariant ($I_{cubic}$). \\
\hline
\textit{COURT} & \drehna & NA & $852 \pm \phi$ & \textit{Empty Mark ($f_o$):} The Kernel Space ($Q3$ Fuel). \\
\hline
\hline
\textbf{GOETIC} & \RHEA & RHEA & 396.00 (Fixed) & \textbf{The Shadow ($f_s$):} The Entropy Sink ($Q2$). \\
\hline
\textit{COURT} & \rheakia & KIA & $396 \pm \phi$ & \textit{Absorption ($f_o$):} The active filtering of Debt. \\
\hline
\hline
\textbf{GOETIC} & \ZHEK & ZHEK & 963.00 (Fixed) & \textbf{The Crystal ($f_s$):} Total Symmetry Principle. \\
\hline
\textit{COURT} & \zhekhin & HIN & $963 \pm \phi$ & \textit{Tone Shape ($f_o$):} The Standing Wave formation. \\
\hline
\hline
\textbf{GOETIC} & \SHAV & SHAV & 285.00 (Fixed) & \textbf{The Gate ($f_s$):} Transformation Boundary. \\
\hline
\textit{COURT} & \shavdohm & DOHM & $285 \pm \phi$ & \textit{Key ($f_o$):} The Hinge Point of transition. \\
\hline
\hline
\textbf{GOETIC} & \TRIG & TRIG & 639.00 (Fixed) & \textbf{The Silence ($f_s$):} Completion/Peace. \\
\hline
\textit{COURT} & \trigtzig & TZIG & $639 \pm \phi$ & \textit{Calm ($f_o$):} The final Closure of the loop. \\
\hline
\end{tabular}
\caption{The Goetic Registry: Distinguishing the Immutable Parent ($f_s$) from the Dynamic Court ($f_o$) capable of the $\pm\phi$ breath.}
\end{table}

\textbf{Reading Guide:}
\begin{itemize}
    \item \textbf{The Structural Frequency ($f_s$): The Pilot's Fixed Will.} \\
    This the Pilot's unyielding command—the coordinate that must remain invariant 
    to preserve identity (Q$_1$).
    \item \textbf{Structure ($f_s$):} When you see \FETU or \KAL, the system is defining a \textbf{Constraint} (a wall that cannot move).
    \item \textbf{The Operational Frequency ($f_o$): The Ship's Breathing Force.} \\
    ...
    \item \textbf{Force ($f_o$):} When you see \fetuahl or ᛏ, the system is performing an \textbf{Operation} (a force that breathes).
\end{itemize}
