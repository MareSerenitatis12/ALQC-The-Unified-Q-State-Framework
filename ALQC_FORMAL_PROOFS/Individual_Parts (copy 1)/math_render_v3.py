...
# ==============================================================================
# 3. DYNAMIC PREAMBLE PARSER (FULL)
# ==============================================================================
...
# ==============================================================================
# 4. CLEANING ENGINE (APPENDED FIXES)
# ==============================================================================
def clean_file(filepath, dynamic_map):
    ...
    for cmd in sorted_cmds:
        glyph = full_map[cmd]
        # [PRESERVED] Added (?![a-zA-Z]) lookahead from V2
        pattern = re.escape(cmd) + r"(?![a-zA-Z])" 
        content = re.sub(pattern, glyph, content)

    content = transmute_math_structure(content)

    # --- PHASE 2: TABLE & STRUCTURE SURGERY (CRITICAL SECTION) ---
    
    # [PRESERVED] Loop to peel off nested text wrappers
    for _ in range(3):
        content = re.sub(r"\(?:text|mathrm|mathbf|mathit|textbf|textit|emph)\s*\{((?:[^{}]|\{[^}]*\})*)\}", r"", content)

    # [APPENDED FIX] Aggressively strip 	ext if it is used as a prefix (e.g. 	extSeed -> Seed)
    # This specifically targets the artifacts like "Seed_Seal oftheDeamonKing = 	extSeed_	extSeal"
    content = re.sub(r"\text(?=[A-Z])", "", content)

    # [APPENDED FIX] Robust removal of table column garbage including decimals (e.g. p{2.5cm}, L2cm)
    # 1. Standard p{2.5cm}
    content = re.sub(r"p\{\d+(?:\.\d+)?[a-z]+\}", "", content)
    # 2. Stray artifacts from nested braces (L2cm, L2.5cm)
    content = re.sub(r"[A-Z]\{?\d+(?:\.\d+)?[a-z]+\}?", "", content)

    # [PRESERVED] Standard formatting cleanup
...
", content)
    ...
    # [PRESERVED] Added Cite support from V2
    content = re.sub(r"\cite\{([^}]*)\}", r"", content)

    # Text Formatting (Left in for safety, though Phase 2 loop handles most)
...
", content)
    ...
    # [PRESERVED] Strip Layout Commands
    content = re.sub(r"\(vspace|hspace|setlength|tabcolsep|noindent|centering|newpage|clearpage)(?:\[[^\]]*\])?(?:\{[^}]*\})?", "", content)
...
", content) 
    ...
    # [PRESERVED] Final Safety Check for backslashes
...
# ==============================================================================
def main():
    print("IGNITION: ALQC FINAL TRANSMUTER (V2 Full + Appended Fixes)")
    
    # [PRESERVED] Original PREABMLE typo as requested
