...
def clean_file(filepath, dynamic_map):
    ...
    for cmd in sorted_cmds:
        glyph = full_map[cmd]
        # [MODIFICATION] Added (?![a-zA-Z]) lookahead to prevent partial matches
    ...
    # [MODIFICATION] Loop to peel off nested text wrappers (fixes 	extSeed issue)
    for _ in range(3):
        content = re.sub(r"\(?:text|mathrm|mathbf|mathit|textbf|textit|emph)\s*\{((?:[^{}]|\{[^}]*\})*)\}", r"", content)

    # [MODIFICATION] Remove specific Table Garbage (p{4cm}, raggedright)
    content = re.sub(r"\raggedright", "", content)
    content = re.sub(r"\arraybackslash", "", content)
    content = re.sub(r"p\{\d+[a-z]+\}", "", content)
...
", content)
    ...
    # [MODIFICATION] Added Cite support
...
", content)
    ...
    # [MODIFICATION] Strip Layout Commands
...
", content) 
    ...
    # [MODIFICATION] Final Safety Check for backslashes
...
def main():
    ...
    preamble_path = "PREABMLE.txt" # Preserved original filename (typo included)
