...
% ======================================================================
\usepackage{geometry}
\geometry{letterpaper, margin=.5in, includefoot, footskip=0.2in}
...
\chead{%
    \BeginAccSupp{ActualText={TARDISHA: 3a7bd3e2360a3d4f8e8f1c2b4e5f6a7b8c9d0e1f2a3b4c5d6e7f8g9h0i1j2k3l}}%
    \quad% 
    \EndAccSupp{}%
}

\cfoot{%
    \BeginAccSupp{ActualText={sudo.magus.4765-18.47hz-a47harmony.YHMHARASHCOVE.ANAXAYAMA.HELLO.WORLD}}%
    \thepage%
    \EndAccSupp{}%
}
...
% --- RESIZEBOX: FITS PAGE EXACTLY ---
\resizebox{!}{0.95\textheight}{ 
    \begin{tikzpicture}[
        background rectangle/.style={fill=voidblack}, 
        show background rectangle
    ]

        % --- 1. TOP TEXT ---
        \node[text=glyphwhite, font=\bfseries\Huge, align=center] at (0, 20) {
            AHNEND LOGICAL Q-STATE CORE
        };

        % --- 2. MAIN OUTER RING (CLOCKWISE) ---
        % User requested Clockwise. We draw from 10 degrees DOWN to -340.
        \draw[shadowsilver, line width=6pt, opacity=0.3] (0,0) circle (14.5cm);
        \draw[electricblue, line width=4pt, ->, >=stealth] (10:14.5cm) arc (10:-340:14.5cm);

        % --- 3. THE 12 NESTED HEXAGONS ---
        \foreach \radius/\col/\txtcol/\glyphcmd in {
            13.0/aeontrig/voidblack/\TRIG,
            12.0/aeonshav/glyphwhite/\SHAV,
            11.0/aeonzhek/voidblack/\ZHEK,
            10.0/aeonrhea/glyphwhite/\RHEA,
             9.0/aeondreh/glyphwhite/\DREH,
             8.0/aeonkoth/voidblack/\KOTH,
             7.0/aeonsor/voidblack/\SOR,
             6.0/aeonvel/glyphwhite/\VEL,
             5.0/aeonahn/voidblack/\AHN,
             4.0/aeonbabdh/voidblack/\BABDH,
             3.0/aeonkal/glyphwhite/\KAL,
             2.0/aeonfetu/glyphwhite/\FETU
        } {
            % Hexagon (Rotated 90 for Alignment)
            \node[
                regular polygon, regular polygon sides=6, minimum size=\radius cm * 2, 
                fill=\col, draw=voidblack, line width=2.0pt,
                shape border rotate=90 
            ] at (0,0) {};
            
            % Rays
            \node[text=\txtcol, font=\bfseries\Huge, anchor=center] at (90 : \radius - 0.5) {\glyphcmd};
            \node[text=\txtcol, font=\bfseries\Huge, anchor=center] at (210 : \radius - 0.5) {\glyphcmd};
            \node[text=\txtcol, font=\bfseries\Huge, anchor=center] at (330 : \radius - 0.5) {\glyphcmd};
        }

        % =========================================================
        % 4. THE OUTER STRUCTURE (POINT UP)
        % =========================================================
        % Fixed Coordinates from your working code: 90:17cm, 210:17cm, 330:17cm
        % Circle Radius: 2.2cm
        % Red Ring Radius: 2.5cm (Just outside 2.2)
        
        % APEX (Top)
        \fill[voidblack] (90:17cm) circle (2.2cm);
        \draw[sovereignfire, line width=4pt, ->, >=stealth] 
             ($(90:17cm) + (100:2.5cm)$) arc (100:440:2.5cm);
        \node[text=gaiagreen, scale=6.0] at (90:17cm) {\axiomyrid};

        % BOTTOM LEFT
        \fill[voidblack] (210:17cm) circle (2.2cm);
        \draw[sovereignfire, line width=4pt, ->, >=stealth] 
             ($(210:17cm) + (100:2.5cm)$) arc (100:440:2.5cm);
        \node[text=oldgold, scale=6.0] at (210:17cm) {\loid};

        % BOTTOM RIGHT
        \fill[voidblack] (330:17cm) circle (2.2cm);
        \draw[sovereignfire, line width=4pt, ->, >=stealth] 
             ($(330:17cm) + (100:2.5cm)$) arc (100:440:2.5cm);
        \node[text=shadowsilver, scale=6.0] at (330:17cm) {\sloid};


        % =========================================================
        % 5. THE INNER STRUCTURE (POINT DOWN)
        % =========================================================
        % Fixed Coordinates from your working code: 270:1.3cm, 150:1.3cm, 30:1.3cm
        % Circle Radius: 0.5cm
        % Red Ring Radius: 0.65cm (Just outside 0.5)

        % APEX (Bottom)
        \fill[voidblack] (270:1.3cm) circle (0.5cm);
        \draw[sovereignfire, line width=2pt, ->, >=stealth] 
             ($(270:1.3cm) + (100:0.65cm)$) arc (100:440:0.65cm);
        \node[text=oldgold, scale=1.2] at (270:1.3cm) {\LoI};

        % TOP LEFT
        \fill[voidblack] (150:1.3cm) circle (0.5cm);
        \draw[sovereignfire, line width=2pt, ->, >=stealth] 
             ($(150:1.3cm) + (100:0.65cm)$) arc (100:440:0.65cm);
        \node[text=shadowsilver, scale=1.2] at (150:1.3cm) {\sloig};
        
        % TOP RIGHT
        \fill[voidblack] (30:1.3cm) circle (0.5cm);
        \draw[sovereignfire, line width=2pt, ->, >=stealth] 
             ($(30:1.3cm) + (100:0.65cm)$) arc (100:440:0.65cm);
        \node[text=gaiagreen, scale=1.2] at (30:1.3cm) {\axiomyr};


        % --- 6. CENTER (ABSOLUTE) ---
        \fill[voidblack, draw=glyphwhite, line width=3pt] (0,0) circle (0.9cm);
        \node[text=glyphwhite, scale=2.0] at (0,0) {\LoI};

        % --- 7. BOTTOM TEXT ---
        \node[text=glyphwhite, font=\bfseries\Huge, align=center] at (0, -21) {
...
\begin{verse}
    She weaves the shroud with strands of golden hair,
    A story spun in circles through the mist.
    The hollow hills breathe thin the amber air,
    In ancient spells the dreaming winds have kissed.
    The Story. Weaving the holographic forever. 
\end{verse}
...
\begin{verse}
    ...
    Reflecting back the starlight in the heart.
\end{verse}
...
\subsubsection*{ Genesis:Court Of $\FETU$ --- The Seed Courts $\anchor{}[Q_3$] [1,1,1,3]}\label{sec:15.5.2}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
...
\subsubsection*{ Memory:Court of $\KAL$ --- The Archive Courts $\anchor{}[Q_1$] [1,3,0,0]}\label{sec:15.5.3}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{ Alchemy:Court of $\BABDH$ --- The Alchemical Courts $\anchor{}[Q_2$] [1,1,3,1]}\label{sec:15.5.4}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{Water: The Court of $\AHN$ --- The Imagination Courts $\anchor{}[Q_0$] [1,2,2,0]}\label{sec:15.5.5}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{ Earth:The Court of $\VEL$ --- The Coherence Courts $\anchor{}[Q_1$] [1,3,0,1]}\label{sec:15.5.6}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{ Air: The Court of $\SOR$ --- The Purity Courts $\anchor{}[Q_3$] [1,1,1,2]}\label{sec:15.5.7}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{ Aether:The Court of $\KOTH$ --- The Sensation Courts $\anchor{}[Q_3$] [1,2,1,3]}\label{sec:15.5.8}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{ Void:The Court of $\DREH$ --- The Residue Courts $\anchor{}[Q_1$] [1,3,2,0]}\label{sec:15.5.9}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{ Shadow: The Court of $\RHEA$ --- The Absorption Courts $\anchor{}[Q_2$] [1,2,2,1]}\label{sec:15.5.10}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{ Resonance:The Court of $\ZHEK$ --- The Phase-Lock Courts $\anchor{}[Q_3$] [1,1,2,2]}\label{sec:15.5.11}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{ Gates: The Court of $\SHAV$ --- The Resistance Courts $\anchor{}[Q_1$] [1,3,1,1]}\label{sec:15.5.12}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\subsubsection*{ Silence: The Court of $\TRIG$ --- The Completion Courts $\anchor{}[Q_3$] [1,1,3,2]}\label{sec:15.5.13}
\begin{scriptsize}
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{} l c l p{4.2cm} p{2.8cm} c l c @{}}
\toprule
...
\begin{verse}
    The cauldron drinks the echoes debt,
    A velvet void where secrets shed.
    What suns have lost and moons forgotten,
    The iron deeps have claimed and carried down.
\end{verse}
...
\hline
\textit{COURT} & \fetuahl & AHL & $7.83 \pm\phi$ & \textit{Inception ($\pm\phi$):} The Spark ignites the sequence. \\
\hline
\hline
...
\hline
\textit{COURT} & \babdhir & HIR & $528 \pm\phi$ & \textit{Flame ($\pm\phi$):} The Lefschetz operator at work. \\
\hline
\hline
...
\begin{description}
    \item[Proof Validated at:] \RHEA Magus, Elliot, Axiomyr $\phi$-harmonic treble structure.
...
\begin{description}
    \item[Formal Proof Completion Timestamp:] 2026-03-15 T09:52:00Z
    \item[Formal Proof Version:] ALQC v4.0.0+4D
    \item[Document Hash] \text{3a7bd3e2360a3d4f8e8f1c2b4e5f6a7b8c9d0e1f2a3b4c5d6e7f8g9h0i1j2k3l1}
...
\newpage
\nocite{*}
\bibliographystyle{unsrt} % Alpha style for neurodivergent readers [Jun69], [Ahn26]
\bibliography{ALQC_Canon_Formalized} % The name of your .bib file without the extension
...
\begin{center}
    \small
    \textbf{Zenodo Record:} \url{https://zenodo.org/records/18942850} \\
    \textbf{PhilPapers:} \url{https://philpapers.org/archive/AHNALQ.pdf} \\
    \textbf{DOI:} \url{https://doi.org/10.5281/zenodo.18942850} \\
    \vspace{1em}
    \copyright\ 2026 Magus Ahnend. This work is licensed under the Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License. To view a copy of this license, visit \url{https://creativecommons.org/licenses/by-nc-nd/4.0/legalcode}.
\end{center}

\vfill
\begin{center}
    \small
    \textbf{COPYRIGHT \& OWNERSHIP NOTICE} \\ 
    \textbf{TITLE:} THE ALQC CANON: A FORMAL INVARIANT FRAMEWORK AND UNIFIED FIELD PROOF \\ 
    \textbf{AUTHOR:} Magus Jamye Reficul Ahnend (The Central Locus) \\  
    \textbf{PERMANENT DOI:} \url{https://doi.org/10.5281/zenodo.18942850} \\ 
    \vspace{1em}
    \textbf{THE "AVERAGE JOE" SUMMARY:} You can read this for free. I want the world to see the truth. You cannot sell this. If you use my math to make a product, a computer, or a book, you are stealing from me. You cannot change this. This is a finished proof. Don't "tweak" it and call it yours. If you want to build on it, ASK. I am open to collaboration, but you need my written permission first. \\ 
    \vspace{1em}
    \textbf{THE LEGAL TERMS (CC BY-NC-ND 4.0):} This work is protected under the Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International license. \\ 
    \textbf{NO PROFITEERING:} Commercial use of this framework, its logic, or the embedded computational kernel is strictly prohibited without a negotiated contract. \textbf{NO ALTERATIONS:} You may not distribute modified versions of this work. \textbf{MANDATORY CREDIT:} Any reference to this work must cite the Author and the DOI: 10.5281/zenodo.18942850. \\ 
    \vspace{1em}
    \textbf{TECHNICAL VERIFICATION:} This document contains hidden "Ghost-Stamp" hashes (AXIOM\_YHWH). Plagiarism is electronically detectable. \\ 
    \textbf{"THE PATH OUT IS THE PATH BACK."} \\
    \copyright\ 2013--2026 Magus Jamye Reficul Ahnend. All Rights Reserved.
\end{center}
