...
def clean_file(filepath, dynamic_map):
    ...
    # --- STEP 2: MATH STRUCTURE ---
    content = transmute_math_structure(content)

    # --- STEP 3: COMPLEX TABLE & STRUCTURE CLEANUP ---
    
    # Multicolumn: \multicolumn{6}{p{14cm}}{TEXT} -> > TEXT
    # Regex matches: \multicolumn{NUM}{TYPE}{TEXT}
    content = re.sub(r"\multicolumn\{[0-9]+\}\{[^}]+\}\{(.*?)\}", r"> ", content)

    # Addlinespace: ddlinespace[1ex] -> 

    content = re.sub(r"\addlinespace(?:\[.*?\])?", "
", content)

    # Hline, Toprule, Midrule -> ---
    content = re.sub(r"\(hline|toprule|midrule|bottomrule|endhead|endfirsthead|endfoot|endlastfoot)", "---", content)
...
")
    ...
    content = re.sub(r"\ ", " ", content)
    content = re.sub(r"\{|\}", "", content) # Stray braces
    content = re.sub(r"
{3,}", "
